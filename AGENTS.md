...
1.0.85. Set `MAGPILOT_RUNTIME_BACKEND=sdk` to select the SDK backend for
ordinary sessions. The default remains `acp`; Agency always remains ACP.
...
| `MAGPILOT_RUNTIME_BACKEND` | agent (optional) | `acp` (default) or `sdk`. Selects the backend for ordinary create/adopt operations. Agency always routes to ACP. Invalid values fail Agent startup explicitly. Changing the value requires an Agent restart; attached sessions never switch backend in place. |
...
- **Wedged-but-alive child (the gap `IsAlive` does NOT close).**
  ...
  **waiting on a tool it invoked** (a long shell command, a slow MCP
  ...
  exempt entirely via per-session open-tool-call tracking, so neither a
  slow model nor a long tool is killed as a false positive; a co-hosted
  session with a still-progressing (or tool-waiting) turn vetoes the
